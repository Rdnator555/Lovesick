...
local Player = enum.PlayerType

---register items to EID
function EIDRegistry.register()
	if EID then
		-- EID function calls here ...
		EID:addCollectible(Item.LooseThread, "Turn some pickups into patches..#Use it mid grab to patch an item. Use it with 5 patches to revive at a Shopkeeper or with 3 to remove a broken heart.", "Locked Heart", en_us)
		EID:addCollectible(Item.SnowballTreat, "{{ArrowUp}}Hp Up. #On new floor gives the i can see forever effet", "Snowball's Treat", en_us)
		EID:addBirthright(Player.Snowball, "Reduce the ammount of patches expend on patching items.#{{ArrowUp}}Minor buff to all patches.#Revive has a 50% of not destroy the shopkeeper ")
